...
def generate_domain_variations(domain, tld, possible_chars):
    unique_domains = set()
    ...
    for i in range(len(domain)):
        new_domain = domain[:i] + domain[i+1:] + '.' + tld
        unique_domains.add(new_domain)
...
# 分割出需要处理的部分和顶级域名(TLD)
domain_part, tld = original_domain.split('.')

# 定义所有可能的字符，0-9和a-z
possible_chars = '0123456789abcdefghijklmnopqrstuvwxyz'

# 生成所有可能的域名变体
unique_domains = generate_domain_variations(domain_part, tld, possible_chars)

# 获取当前工作目录
current_dir = os.getcwd()

# 输出所有独特的域名到文件
output_file_path = os.path.join(current_dir, 'redteam_evil_domain.txt')
